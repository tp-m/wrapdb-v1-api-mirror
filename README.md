...
nginx server config (as-is), contains

 - home page redirect and v2 api redirects to GitHub (not of interest)
 - legacy v1 api handling via uswgi (to be removed)
 - decorate static files served from `/v1-static/` with the right file types and headers
 - redirects from `/x1/*` api prefix to `/v1-static`
   - change to `/v1/` -> `/v1-static/` once it's ready to go live
   - `x1` api prefix allows testing the mirror script both against the
     still-live legacy `v1` api and the fake `x1` api (can change the `api`
     variable in the script) to make sure the downloaded results are identical.
